...
static const char *optype(const std::string &s) {
  if (s == "AL")
    return "AL";
  if (s == "AX")
    return "AX";
...
int main() {
  std::string name, lhs, rhs, byte, bytes, flags, last;

  std::ifstream in("nasm_insts.txt");
  std::ofstream out("../compiler/assem_x86/asm_insts.cpp");
  ...
  for (; !in.eof(); in.ignore(INT_MAX, '\n')) {
    ...
    bytes = "\\x" + std::to_string(16) + bytes;
