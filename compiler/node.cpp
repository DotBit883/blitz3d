...
TNode *Node::createVars(Environ *e) {
  ...
  for (k = 0; k < e->decls->size(); ++k) {
    ...
    TNode *m =
        d->kind == DECL_GLOBAL ? global("_v" + d->name) : local(d->offset);
    p = move(p, mem(m));
    if (t)
      t = seq(t, p);
    else
...
TNode *Node::seq(TNode *l, TNode *r) { return new TNode(IR_SEQ, l, r); }
...
TNode *Node::local(int offset) { return new TNode(IR_LOCAL, 0, 0, offset); }

TNode *Node::arg(int offset) { return new TNode(IR_ARG, 0, 0, offset); }

TNode *Node::mem(TNode *ref) { return new TNode(IR_MEM, ref, 0); }

TNode *Node::add(TNode *l, TNode *r) { return new TNode(IR_ADD, l, r); }

TNode *Node::mul(TNode *l, TNode *r) { return new TNode(IR_MUL, l, r); }

TNode *Node::iconst(int n) { return new TNode(IR_CONST, 0, 0, n); }

TNode *Node::ret() { return new TNode(IR_RET, 0, 0); }

TNode *Node::jsr(const std::string &s) { return new TNode(IR_JSR, 0, 0, s); }

TNode *Node::jump(const std::string &s) { return new TNode(IR_JUMP, 0, 0, s); }
