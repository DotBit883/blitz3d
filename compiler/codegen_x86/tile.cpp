...
static const std::string regs[] = {"???", "eax", "ecx", "edx",
                                   "edi", "esi", "ebx"};
...
static void freeReg(int n) { regUsed[n] = false; }
...
Tile::Tile(const std::string &a, Tile *l, Tile *r)
    : assem(a), l(l), r(r), want_l(0), want_r(0), hits(0), need(0),
      argFrame(0) {}

Tile::Tile(const std::string &a, const std::string &a2, Tile *l, Tile *r)
    : assem(a), assem2(a2), l(l), r(r), want_l(0), want_r(0), hits(0), need(0),
      argFrame(0) {}
...
void Codegen_x86::align_data(int n) {
  dataFrags.push_back(std::string("\t.align\t") + itoa(n) + '\n');
}

void Codegen_x86::i_data(int i, const std::string &l) {
  if (l.size())
    dataFrags.push_back(l);
  dataFrags.push_back(std::string("\t.dd\t") + itoa(i) + '\n');
}
