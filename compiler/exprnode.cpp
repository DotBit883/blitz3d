...
TNode *VarExprNode::translate(Codegen *g) { return var->load(g); }
...
int FloatConstNode::intValue() {
  fenv_t old_fenv;
  if (fegetenv(&old_fenv) != 0) {
    // In a noexcept function, you can't throw. Log an error or use an
    // assertion.
    throw std::runtime_error("Warning: Could not save FPU environment.");
  }

  // Set rounding mode to round-to-nearest
  // FE_TONEAREST is the standard round-to-nearest mode.
  if (fesetround(FE_TONEAREST) != 0) {
    throw std::runtime_error("Warning: Could not set FPU rounding mode.");
  }

  // Disable floating-point exceptions as per original _control87 flags.
  // FE_ALL_EXCEPT includes FE_INVALID, FE_DIVBYZERO, FE_OVERFLOW, FE_UNDERFLOW,
  // FE_INEXACT. FE_DENORMAL is not directly part of FE_ALL_EXCEPT in all fenv.h
  // implementations, but often handled implicitly or not directly maskable via
  // feenableexcept/fedisableexcept. We disable the common ones that map
  // directly.
  int exceptions_to_disable =
      FE_INVALID | FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW | FE_INEXACT;

  // Save current exception mask and then disable the specified exceptions.
  // This ensures that these exceptions do not cause a signal/trap.
  if (fedisableexcept(exceptions_to_disable) != 0) {
    throw std::runtime_error("Warning: Could not disable FPU exceptions.");
  }

  // Perform the conversion: round to nearest, then cast to int.
  // std::round() performs round-to-nearest.
  int temp = static_cast<int>(std::round(value));

  // Restore original floating-point environment
  if (fesetenv(&old_fenv) != 0) {
    throw std::runtime_error("Warning: Could not restore FPU environment.");
  }
  return temp;
}
  ...
  if (ConstNode *c = expr->constNode()) {
    ...
    if (sem_type == Type::int_type) {
      ...
      case '-':
        ...
        break;
      case ABS:
        e = new IntConstNode(c->intValue() >= 0 ? c->intValue()
                                                : -c->intValue());
        break;
      case SGN:
        e = new IntConstNode(c->intValue() > 0 ? 1
                                               : (c->intValue() < 0 ? -1 : 0));
        break;
      }
    } else {
      ...
      case '-':
        ...
        break;
      case ABS:
        e = new FloatConstNode(c->floatValue() >= 0 ? c->floatValue()
                                                    : -c->floatValue());
        break;
      case SGN:
        e = new FloatConstNode(
...
TNode *NullNode::translate(Codegen *g) { return new TNode(IR_CONST, 0, 0, 0); }
...
TNode *ObjectCastNode::translate(Codegen *g) {
  TNode *t = expr->translate(g);
  t = call("__bbObjFromHandle", t,
           global("_t" + sem_type->structType()->ident));
  return t;
}
